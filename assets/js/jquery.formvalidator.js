            ...
            if (errorInputs.length > 0) {
                ...
                // Display error message below input field
                    ...
                    for (var i = 0; i < errorInputs.length; i++) {
                        ...
                        var errorSpan = parent.find('span[class=alert]');
...
jQueryFormUtils.validateFutureDate = function(val, dateFormat) {
	var inputDate = this.parseDate(val, dateFormat);
	if (!inputDate) {
        return false;
    }
 ...
 * @return {Boolean}
 */
jQueryFormUtils.validateNum = function(val) {
...
jQueryFormUtils.validate = function(value, el, language, config, form) {
    ...
    if (typeof validationRules != 'undefined' && validationRules !== null) {
		...
		// Num
        else if (validationRules.indexOf('validate_num') > -1 && !jQueryFormUtils.validateNum(value)) {
            return validationErrorMsg || language.badNum;
        }
		...
		// Future date
        else if (validationRules.indexOf('validate_future_date') > -1 && !jQueryFormUtils.validateFutureDate(value, config.dateFormat)) {
            return validationErrorMsg || language.badFutureDate;
        }
...
jQueryFormUtils.LANG =  {
    errorTitle : 'Feil',
    requiredFields : 'P̴krevde felt m̴ fylles ut',
    badTime : 'Ikke gyldig tidspunkt',
    badEmail : 'Ikke gyldig epostadresse',
    badTelephone : 'Ikke gyldig telefonnummer',
    badSecurityAnswer : 'You have not given a correct answer to the security question',
    badDate : 'Ikke gyldig dato',
	badFutureDate : 'Velg en fremtidig dato',
	...
	badNum: 'Skriv inn et tall',
    badCustomVal : 'You gave an incorrect answer',
    badInt : 'Skriv inn et heltall',
